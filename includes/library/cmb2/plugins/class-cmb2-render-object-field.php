 ...
 */
class CMB2_Render_Object_Field extends CMB2_Type_Base {
	/**
	...
	public static function init() {
		add_filter( 'cmb2_render_class_object', array( __CLASS__, 'class_name' ) );
		add_filter( 'cmb2_sanitize_object', array( __CLASS__, 'maybe_save_split_values' ), 12, 4 );

		/**
		 * The following snippets are required for allowing the address field
		 * to work as a repeatable field, or in a repeatable group
		 */
		add_filter( 'cmb2_sanitize_object', array( __CLASS__, 'sanitize' ), 10, 5 );
		add_filter( 'cmb2_types_esc_object', array( __CLASS__, 'escape' ), 10, 4 );
	}
			...
			<table cellspacing="0" cellpadding="10" class="cmb_table_field">
				<tbody>
					<tr class="rc_wc_first_row">
						...
						<td>
							<p><label for="<?php echo $this->_id( '_obj_name', false ); ?>'"><?php echo esc_html( $this->_text( 'material_obj_name_text', 'Object Name to Show' ) ); ?></label></p>
							<?php echo $this->types->input( array(
