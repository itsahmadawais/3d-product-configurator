...
/**
 * Handles 'choices' custom field type.
 * This custom input is actually a group input type with the following input fields:
 * 🔴 Title
 * 🔴 choices
 * 🔴 Price
 * This data does not affect anything in the frontend.
 */
class CMB2_Render_Choices_Field extends CMB2_Type_Base {
	...
	public static function init() {
		add_filter( 'cmb2_render_class_choices', array( __CLASS__, 'class_name' ) );
		add_filter( 'cmb2_sanitize_choices', array( __CLASS__, 'maybe_save_split_values' ), 12, 4 );

		/**
		 * The following snippets are required for allowing the address field
		 * to work as a repeatable field, or in a repeatable group
		 */
		add_filter( 'cmb2_sanitize_choices', array( __CLASS__, 'sanitize' ), 10, 5 );
		add_filter( 'cmb2_types_esc_choices', array( __CLASS__, 'escape' ), 10, 4 );
	}
	...
	public static function maybe_save_split_values( $override_value, $value, $object_id, $field_args ) {
		...
		$material_keys = array( 'mat_title', 'mat_price', 'map' );
		...
		remove_filter( 'cmb2_sanitize_choices', array( __CLASS__, 'sanitize' ), 10, 5 );
