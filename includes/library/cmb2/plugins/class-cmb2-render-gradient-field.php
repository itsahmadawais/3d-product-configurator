...
/**
 * Handles 'gradient' custom field type.
 */
class CMB2_Render_Gradient_Field extends CMB2_Type_Base {
	...
	public static function init() {
		add_filter( 'cmb2_render_class_gradient', array( __CLASS__, 'class_name' ) );
		add_filter( 'cmb2_sanitize_gradient', array( __CLASS__, 'maybe_save_split_values' ), 12, 4 );

		/**
		 * The following snippets are required for allowing the address field
		 * to work as a repeatable field, or in a repeatable group
		 */
		add_filter( 'cmb2_sanitize_gradient', array( __CLASS__, 'sanitize' ), 10, 5 );
		add_filter( 'cmb2_types_esc_gradient', array( __CLASS__, 'escape' ), 10, 4 );
	}
			...
			<table cellspacing="0" cellpadding="10" class="cmb_table_field">
				<tbody>
					<tr class="rc_wc_first_row">
						<td>
							<p><label for="<?php echo $this->_id( '_mat_title', false ); ?>'"><?php echo esc_html( $this->_text( 'gradient_title_text', 'Title' ) ); ?></label></p>
						...
						<td>
							<p><label for="<?php echo $this->_id( '_color', false ); ?>'"><?php echo esc_html( $this->_text( 'gradient_color_text', 'Color' ) ); ?></label></p>
	...
	public static function maybe_save_split_values( $override_value, $value, $object_id, $field_args ) {
		...
		$gradient_keys = array( 'mat_title', 'color' );
		...
		remove_filter( 'cmb2_sanitize_gradient', array( __CLASS__, 'sanitize' ), 10, 5 );
